...
•	Cantidad de menores unidos por el matrimonio a pesar de que existen leyes en Guatemala que prohíben esto. 


•	Estándares de los matrimonios en Guatemala. 
...
1. Determinar la cantidad de menores casados y su respectiva información.
2. Determinar la tendencia de los matrimonios en Guatemala.


General:

1. Determinar el cumplimiento de las leyes matrimoniales en Guatemala
...
**`r names(db)`**. Las cuales se poseen las siguientes variables numericas y categoricas. Donde las unicas numericas que presentamos son las edades de los individuos.
...
### Analisis Exploratorio, sección menores de edad. 
...
PreguntaEscolaridad<-ggplot(data=EscoH, aes(x=EscolaridadHombre, y=CantidadEscolaridadHombre,fill=EscolaridadHombre)) +
  ...
  geom_text(aes(label=CantidadEscolaridadHombre), vjust=1.6, color="black",
            position = position_dodge(0.9), size=3.5)+
  labs(title="Nivel de escolaridad de los hombres con matrimonio", y="Cantidad de matrimonios ")+
  theme(legend.position="none")
...
PreguntaEscolaridadMujer<-ggplot(data=EscoM, aes(x=EscolaridadMujer, y=CantidadEscolaridadMujer,fill=EscolaridadMujer)) +
  ...
  geom_text(aes(label=CantidadEscolaridadMujer), vjust=1.6, color="black",
            position = position_dodge(0.9), size=3.5)+
  labs(title="Nivel de escolaridad de las mujeres", y="Cantidad")+
  theme(legend.position="none")
#Pie chart
...
pieEscM<-ggplot(EscoM, aes(x = "", y = pdM, fill = EscolaridadMujer)) +
  ...
  geom_label(aes(label = pdM),
             ...
             show.legend = FALSE) +
  coord_polar(theta = "y")+theme(legend.position = "bottom")+
  labs(title="Porcentaje de escolaridad de los mujeres")
...
diffEdades<-ggplot(data=diffMayorN, aes(x=Diferencia, y=Diferencia,fill=Diferencia)) +
  ...
  geom_text(aes(label=Diferencia), vjust=1.6, color="black",
            position = position_dodge(0.9), size=3.5)+
  labs(title="Mayores diferencias de edades de personas casadas con un menor", y="Diferencia de edades")+
  theme(legend.position="none")
...
diffEdadesG<-ggplot(data=diffGroup, aes(x=Diferencia, y=n,fill=Diferencia)) +
  ...
  geom_text(aes(label=n), vjust=1.6, color="black",
            position = position_dodge(0.9), size=3.5)+
  labs(title="Cantidad de años de diferencia de edades entre personas casadas con un menor", y="Diferencia de edades")+
  theme(legend.position="none")
...
Tal como se observa del **100%** de los matrimonios donde exista algun menor el **88.66%** es cuando la mujer es menor de edad y el hombre es mayor que ella, el **7.68%** es cuando el hombre es menor de edad y el **3.67%** cuando ambos individuos comparten la misma edad y tambien son menores de edad. Ahora si nos enfocamos en la diferencia entre las edades se obtiene lo siguiente

```{r}
diffEdades
```
Como se observa la mayor diferencia entre un menor de edad y su pareja es de 62 años y siguiendole la edad de 59 años. Ahora si vemos las edades agrupadas obtenemos lo siguiente.
...
Tal como se ve en la grafica la mayor diferencia de edades entre un menor de edad y su pareja es de 3 años, y en segundo lugar es de 2 años.
```{r}
...
Como se observa en la grafica se demuestra los mujeres casados  tienen el siguiente grado academico:   
\n
...
### Analisis Exploratorio, sección de tendencias. 
...
pregunta2.2<-ggplot(data=df1, aes(x=reorder(Mes,Cantidad), y=Cantidad, fill=Mes)) +
  ...
  geom_text(aes(label=Cantidad), vjust=1.6, color="black",
            position = position_dodge(0.9), size=3.5)+
  labs(title="Cantidad de bodas registradas por mes", x="Mes", y="Cantidad")+theme(legend.position="none")
...
pregunta1EH<-ggplot(data=df, aes(x=reorder(Municipio,CantidadMa), y=CantidadMa, fill=Municipio)) +
  geom_bar(stat="identity", position=position_dodge())+
  geom_text(aes(label=CantidadMa), vjust=1.6, color="black",
            position = position_dodge(0.9), size=3.5)+
...
pregunta2EH<-ggplot(data=df2, aes(x=reorder(Municipio,CantidadMa), y=CantidadMa, fill=Municipio)) +
  geom_bar(stat="identity", position=position_dodge())+
  geom_text(aes(label=CantidadMa), vjust=1.6, color="black",
            position = position_dodge(0.9), size=3.5)+
...
pregunta3EH<-ggplot(EtnH, aes(x = "", y = pd, fill = EtniaHombre)) +
  geom_col(color = "black") +
  geom_label(aes(label = pd),
             position = position_stack(vjust = 0.5),
             show.legend = FALSE) +
...
#Pie chart
pdM<-round(((CantidadEtniaMujer*100)/sum(EtnM$CantidadEtniaMujer)),digits = 2)

pregunta4EH<-ggplot(EtnM, aes(x = "", y = pdM, fill = EtniaMujer)) +
  geom_col(color = "black") +
  geom_label(aes(label = pdM),
             position = position_stack(vjust = 0.5),
             show.legend = FALSE) +
...
pregunta5EH<-ggplot(data=df5, aes(x=Año, y=EtnPromedio, fill=Sexo)) +
  geom_bar(stat="identity", position=position_dodge())+
...
pregunta6EH<-ggplot(mismaEtnia, aes(x = "", y = pe, fill = igual)) +
  geom_col(color = "black") +
...
Como se puede observar en la grafica, el departamento que cuenta con una clara diferencia por sobre los demas departamentos es el #1, el cual corresponde al departamendo de Guatemala con 168220 bodas registradas, el cual es aproximadamente tres veces mas que el departamento que le sigue, como lo es el departamento de Huehuetenango, que tiene una cantidad de matrimonios registrados de: 61839. 
...
En el gráfico podemos observar que el municipio donde se han realizado más bodas es en el municipio 0101 que corresponde al municipio de Guatemala del departamento de Guatemala, y le sigue San José Pinula, Guatemala; Santa Catarina Pinula, Guatemala; San José del Golfo; Guatemala y San Raymundo; Guatemala. Todos pertenecientes al departamento de Guatemala.

```{r}
pregunta2EH
```

En este gráfico se observa cinco municipios que han tenido solo una boda, estos son: Totonicapán, Santa Catarina Palopó, Guazacapán, Palín y Santa María de Jesús. Cabe mencionar que hay otros municipios donde se han realizado una única boda y departamentos donde no se han registrado ni una boda.
...
En este gráfico podemos observar los 5 días donde se registraron mas matrimonios en el mismo día.
...
La escolaridad promedio por año, tanto de hombres como de mujeres tiene en como claro ganador el 3, lo que hace referencia a un grado de escolaridad de Basico, seguido por dos años atipicos, los cuales justamente son el 2010 y el año 2020, que se tiene un promedio de escolaridad de , que tienen como significad que para esos años, el grado Diversificado era el que dominaba tanto en hombres como en mujeres. 
...
El metodo de union mas comun de ver en los 10 años analizados, es el metodo numero 3 o la opcion 3, el cual hace referencia a COMUNIDAD DE GANANCIALES, esto quiere decir que durante su vigencia sólo uno de los cónyuges administra los bienes comunes, pero al término de la sociedad se dividen los gananciales por mitad a ambos., que como se explica anteriormente es la Comunidad de gananciales es la clara ganadora, por una cantidad abrumadora, seguido de la clase de union 1, que es la COMUNIDAD ABSOLUTA y la numero 9 que es NO ESPECIFICADO respectivamente.
...
Como dato curioso investigamos la cantidad de extranjeros registrados en algun matrimonio en Guatemala y en la grafica podemos observar que existen 7402 hombres extranjeros y 8174 mujeres extranjeras siendo un total de 15576 personas extranjeras registradas en algun matrimonio en Guatemala
...
Del gráfico anterior se observa que la mayoría, un 39.8%, de los hombres casados son mestizos o ladinos. 35.96% son mayas, 12.68% son garifunas, 1.11% son de otras etnias y el 10.45% son datos ignorados. Entonces podemos decir que la gran parte de los hombres son mestizos o ladinos y mayas.
...
En el caso de las mujeres, 38.48% son mestizas o ladinas, 34.36% son mayas, 12.26% son garifunas, 1.22% son de otras etnias y el 13.68% de los datos son ignorados. Muy similar al caso de los hombres, las mayoría de las mujeres son mestizas o ladinas y mayas.
...
En este gráfico se observa que las bodas realizadas por año, en su mayoría involucraban hombres y mujeres de 3 etnias distintas, con excepción en los años 2014 y 2015 donde involucraban mujeres de 4 etnias distintas, y en el año 2010 involucraba tanto hombres como mujeres de 4 distintas etnias.

```{r}
pregunta6EH
```

De este gráfico se concluye que el 78.32% de las bodas, realizadas desde el 2010 hasta el 2020 y que fueron registradas, fueron entre personas de la misma etnia. Y el 21.68% fueron entre diferentes etnias.
...
1.  El departamento de Guatemala con diferencia es el que registra más matrimonios entre menores con 16822. Este dato contradice el decreto 13-2017 en el cual el congreso de la república establece "Prohibición de contraer matrimonio. No podrán contraer matrimonio ni autorizarse de manera alguna el matrimonio de menores de dieciocho (18) años de edad" de esta forma se elimina el antiguo decreto de 2015 el cual contemplaba el matrimonio entre menores según circunstancias concretas. A pesar de lo anterior, el estudio muestra que la tendencia ha disminuido poco a poco pero aún es un problema social por abordar.


2. Se determino que la mayoría de los matrimonios con menores de edad involucran hombres mayores que mujeres (88.66%), con una diferencia de edad comúnmente reportada de 2 a 5 años. Si bien es cierto que estas uniones pueden implicar una brecha significativa de poder y experiencia, es importante destacar que no siempre se deben a abuso o explotación. De hecho, algunas menores pueden optar por casarse por razones económicas o por estar en una relación juvenil. En cualquier caso, es fundamental abordar esta cuestión para garantizar el bienestar y la protección de los menores de edad, y ofrecer alternativas para que las niñas y jóvenes tengan opciones y oportunidades.

3. Existe desigualdad de género en los casamientos de personas menores de edad, en la mayor parte de casamientos de menores de edad, el matromonio de hombres menores de edad equivale aproximandamente a una quinta parte de la cantidad de mujeres que se casan siendo menores de edad. Además, la probabilidad de que las menores de edad que contraen matrimonio sigan estudiando es mucho menor, basandose en el hallazgo que la mayor parte de menores de edad no tienen ningún nivel académico o apenas tienen el nivel académico de primaria.
