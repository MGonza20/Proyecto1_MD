...
```{r}
merged_dataset_wo999 <- subset(merged_dataset, EDADHOM != 999)

boxplot(merged_dataset_wo999$EDADHOM)
```
Se puede ver que la data de edad de los hombres no está distribuida de forma normal ya que hay varias puntos atipicos.

```{r}
summary(merged_dataset_wo999$EDADHOM)
```



```{r}
lillie.test(merged_dataset_wo999$EDADHOM)
```
Esto se ve confirmado con la prueba de lillie fors ya que el valor es de 0.17 que es menor a 0.5 y esto nos indica que no se sigue una distribución normal.

```{r}

dataframe <- data.frame(
  edad_hombre = merged_dataset_wo999$EDADHOM,
  count = rep(1, nrow(merged_dataset_wo999))
)


ggplot(dataframe, aes(x = edad_hombre)) +
  geom_bar() +
  ggtitle("Distribución de edades del hombre al momento de contraer matrimonio") +
  labs(x = "Edad del hombre", y = "Cantidad de casos") +
  theme_minimal()
...
```{r}
merged_dataset_wo_m999 <- subset(merged_dataset, EDADMUJ != 999)

boxplot(merged_dataset_wo_m999$EDADMUJ)
```
Se puede ver que la data de edad de las mujeres no está distribuida de forma normal ya que hay varias puntos atipicos.




```{r}
lillie.test(merged_dataset_wo_m999$EDADMUJ)
```
Esto se ve confirmado con la prueba de lillie fors ya que el valor es de 0.17 que es menor a 0.5 y esto nos indica que no se sigue una distribución normal.

```{r}
summary(merged_dataset_wo_m999$EDADMUJ)
```


```{r}

merged_dataset_wo_hm999 <- subset(merged_dataset, EDADMUJ != 999) 
merged_dataset_wo_hm999 <- subset(merged_dataset_wo_hm999, EDADHOM != 999) 


ggplot(merged_dataset_wo_hm999, aes(x = EDADHOM, y = EDADMUJ)) +
  geom_point() +
  ggtitle("Relación entre la edad del hombre y la edad de la mujer en el matrimonio") +
  labs(x = "Edad del hombre", y = "Edad de la mujer") +
  theme_minimal()
```
